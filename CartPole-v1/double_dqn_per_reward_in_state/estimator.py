    ...
    def _build_model(self, name):
        with tf.name_scope(name):
            ...
            with tf.name_scope("hidden1"):
                self.W1 = tf.Variable(tf.truncated_normal([5, 256], stddev=0.1), name="W")
                self.b1 = tf.Variable(tf.zeros([256]), name="b")
                self.hidden1 = tf.nn.tanh(tf.matmul(self.input, self.W1) + self.b1)

            with tf.name_scope("hidden2"):
                self.W2 = tf.Variable(tf.truncated_normal([256, 256], stddev=0.1), name="W")
                self.b2 = tf.Variable(tf.zeros([256]), name="b")
                self.hidden2 = tf.nn.tanh(tf.matmul(self.hidden1, self.W2) + self.b2)

            with tf.name_scope("output"):
                self.W3 = tf.Variable(tf.truncated_normal([256, 2], stddev=0.1), name="W")
